...
class ConfusionMatrixManager:
    ...
    def table_divide_and_format(self, table):
        ...
        self.given_label_list = [row[1] for row in table]
    ...
    def get_avg_preds(self):
        ...
        np_table = self.table_divide_and_format(sample_table)

        w1, w2, w3 = 0.5, 0.6, 0.7
        weights_label_a = np.array([w1, 0, w2, 0, w3, 0])
        weights_label_b = np.array([0, w1, 0, w2, 0, w3])

        probs_a = np.sum(np_table*weights_label_a, axis=1)/3
        probs_b = np.sum(np_table*weights_label_b, axis=1)/3

        # tests for calculation
        # print("Test 0 - : ", np_table*weights_label_a)
        # print("Test 1 - : ",  probs_a)
        # print("Pred res: ", get_pred_list(probs_a, probs_b))
        return get_pred_list(probs_a, probs_b)
...
cmm.table_divide_and_format(sample_table)
print("TEST 0 - given_label_list: ", cmm.given_label_list)
print("TEST 1 - prob values: ", cmm.prob_values)

#preds = get_pred_list(get_avg_preds())
...
def get_pred_list(label1_pred, label2_pred):
    res = label1_pred - label2_pred
    return ["A" if x>=0 else "B" for x in res]
